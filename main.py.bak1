...
RATINGS_FILE = 'stock_ratings.json'          # 新增评级文件
...
def generate_dashboard(today_date, now_time):
    ...
    for s in daily.get('left', []):
        code = s['code']
        rating_info = ratings.get(code, {})
        rating_grade = rating_info.get('rating', 'N/A')
        full_report = rating_info.get('full_report', '无详细报告')
        candidates_rows += f"""
        <tr>
            <td>{code}</td>
            <td>{s['name']}</td>
            ...
            <td>{s['bias_val']:.2f}</td>
            <td>{s['low']:.2f}</td>
            <td>{s.get('eps', 0.0):.3f}</td>
            <td>
                <span class="badge bg-info" style="cursor:pointer" data-bs-toggle="modal" data-bs-target="#ratingModal"
                      data-code="{code}" data-name="{s['name']}" data-rating="{rating_grade}" data-detail="{full_report.replace('"', '&quot;')}">
                    {rating_grade}
                </span>
            </td>
        </tr>"""
    if not daily.get('left'):
        candidates_rows = "<tr><td colspan='7'>暂无抄底信号</td></tr>"

    # 历史池表格（增加买入时评级和最新评级两列）
    history_rows = ""
    for rec in history_sorted:
        ...
        rating_at_buy = rec.get('rating_at_buy', 'N/A')
        buy_rating_detail = rec.get('rating_detail_at_buy', '无')
        latest_rating_info = ratings.get(code, {})
        latest_rating = latest_rating_info.get('rating', 'N/A')
        latest_rating_detail = latest_rating_info.get('full_report', '暂无最新报告')
        ...
        <tr>
            ...
            <td>{rec['name']}</td>
            <td>{buy_price:.2f}</td>
            <td>{rec.get('buy_day_low', buy_price):.2f}</td>
            <td>{latest_price:.2f}</td>
            <td class='{color}'>{pct:+.2f}%</td>
            <td>{rec.get('eps', 0.0):.3f}</td>
            <td>
                <span class="badge bg-secondary" style="cursor:pointer" data-bs-toggle="modal" data-bs-target="#ratingModal"
                      data-code="{code}" data-name="{rec['name']}" data-rating="{rating_at_buy}" data-detail="{buy_rating_detail.replace('"', '&quot;')}">
                    {rating_at_buy}
                </span>
            </td>
            <td>
                <span class="badge bg-info" style="cursor:pointer" data-bs-toggle="modal" data-bs-target="#ratingModal"
                      data-code="{code}" data-name="{rec['name']}" data-rating="{latest_rating}" data-detail="{latest_rating_detail.replace('"', '&quot;')}">
                    {latest_rating}
                </span>
            </td>
            <td>{status}</td>
    ...
    if not history_sorted:
        history_rows = "<tr><td colspan='12'>暂无历史记录</td></tr>"

    html = f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>左侧抄底看板（AI增强）</title>
...
<div class="card mb-4">
    <div class="card-header bg-primary text-white">今日抄底候选</div>
    <div class="card-body">
        <table class="table table-hover">
            <thead>
                <tr><th>代码</th><th>名称</th><th>最新价</th><th>乖离率%</th><th>当日最低价</th><th>每股收益</th><th>AI评级</th></tr>
            </thead>
...
<div class="card">
    <div class="card-header bg-secondary text-white">历史追溯池</div>
    <div class="card-body">
        <table class="table table-hover">
            <thead>
                <tr><th>买入日期</th><th>代码</th><th>名称</th><th>买入价</th><th>生命线</th><th>最新价</th><th>涨跌%</th><th>每股收益</th><th>买入时评级</th><th>最新评级</th><th>状态</th><th>卖出原因</th></tr>
            </thead>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ratingModalLabel">股票评级详情</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="ratingModalBody">加载中...</div>
...
<script>
    ...
    ratingModal.addEventListener('show.bs.modal', function (event) {
        ...
        var modalBody = ratingModal.querySelector('.modal-body');
        modalBody.innerHTML = `<h5>${code} ${name}</h5><p><strong>评级：</strong> ${rating}</p><hr/><pre style="white-space:pre-wrap;">${detail}</pre>`;
    });
</script>
</body>
</html>"""
                ...
                for c in yest.get('left', []):
                    if not any(r['code'] == c['code'] and r['buy_date'] == yest['date'] for r in history):
                        ...
                        history.append({
                            ...
                            'eps': c.get('eps', 0.0),
                            'rating_at_buy': rating_info.get('rating', 'N/A'),
                            'rating_detail_at_buy': rating_info.get('full_report', '无')
                        })
                if history:
